...
class Cc65Backend:
    ...
    def _emit_cc65_bkg_engine(self, prof):
        ...
        # so a SINGLE strip placed at hpos = -x covers the screen at *any*
        # horizontal scroll (x = 0..255) with no wrap copy: x + screen <= 255 +
        # sw px must fit, i.e. ceil((255 + sw) / 8) tiles. This keeps the draw at
        # one SCB per row (= the smooth vertical case) instead of doubling to a
        # wrap copy per row during horizontal scroll, which overran Suzy and
        # flickered foreground sprites on real hardware.
        strip_w = (255 + sw + 7) // 8       # strip width in tiles (>= scroll + screen)
        strip_bytes = strip_w * 2 + 2       # offset byte + strip_w cols * 2 + pad
        ...
        self.emit("   recomposites ONE strip per tile boundary -- no full-window recomposite. */")
        ...
        self.emit("}")
        self.emit("/* Composite one screen-spanning row strip `p` from logical map row")
        self.emit("   `map_row`. The strip is STRIP_W tiles wide (> the 32-tile map), so map")
        self.emit("   columns repeat (col c shows map col c & 31) -- that built-in horizontal")
        self.emit("   wrap is what lets a single strip cover any scroll with no wrap copy.")
        self.emit("   Tiles are 4px-aligned (2 bytes wide), so cells land on byte boundaries")
        self.emit("   -- no bit-shifting; the sprite-row base is hoisted out of the row loop. */")
        ...
        self.emit("    for (cx = 0; cx < GBS_BKG_STRIP_W; ++cx) {")
        self.emit("        src = gbs_bkg_tileset[map_r[cx & 31]];")
    ...
    def _emit_cc65_sprite_engine(self, prof):
        ...
        if with_bkg:
            ...
            self.emit("        /* Draw the row-strip ring: ONE screen-spanning strip per visible")
            self.emit("           map row, positioned by (hpos = -x, vpos = row*8 - frac). The")
            self.emit("           strip spans the whole scroll period + the screen, so horizontal")
            self.emit("           scroll is pure hpos -- no recomposite and no wrap copy (one SCB")
            self.emit("           per row, like the vertical case). Vertical scroll recomposites")
            self.emit("           at most ONE strip (the row newly scrolled in); the rest move. */")
            self.emit("        uint8_t i;")
        ...
        self.emit("            if (gbs_scb[s].vpos >= %d || gbs_scb[s].hpos >= %d ||" % (sh, sw))
        self.emit("                gbs_scb[s].vpos <= -8 || gbs_scb[s].hpos <= -8) continue;")

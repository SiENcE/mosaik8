...
class Cc65Backend:
    ...
    def _emit_cc65_bkg_engine(self, prof):
        ...
        self.emit("#define GBS_BKG_STRIP_BYTES %d  /* offset + STRIP_W*2 literal bytes + pad */"
                  % strip_bytes)
        self.emit("#define GBS_BKG_AMORT   12  /* strip columns composited per frame (amortize) */")
        ...
        self.emit("    uint16_t t; uint8_t row, s;")
        self.emit("    gbs_bkg_init();")
        self.emit("    for (t = 0; t < count; ++t)")
        self.emit("        for (row = 0; row < 8; ++row)")
        self.emit("            gbs_bkg_pack_row(data[t*16+row*2], data[t*16+row*2+1], &gbs_bkg_tileset[first+t][row*2]);")
        self.emit("    for (s = 0; s < GBS_BKG_STRIPS; ++s) gbs_bkg_strip_row[s] = 0xFF;  /* recompose */")
        self.emit("}")
        self.emit("/* GB semantics: map coords wrap mod 32; `tiles` is row-major w x h.")
        self.emit("   Writes the logical tile map and marks every strip stale so it")
        self.emit("   recomposites from the new map. */")
        ...
        self.emit("    for (cx = 0; cx < GBS_BKG_STRIPS; ++cx) gbs_bkg_strip_row[cx] = 0xFF;")
        ...
        self.emit("   scroll with no wrap copy. Cells are 4px-aligned (2 bytes wide) so they")
        self.emit("   land on byte boundaries. Composing by column range lets a strip be built")
